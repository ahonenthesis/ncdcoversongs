% ppm-based pairwise distance matrix calculation.
% 
% input:
...
%
function ppmNCD(targetList, queryList, seqdir, otimat, tdir, outputFile)
...
for ix=1:qlen
    ...
    for jx=1:tlen
        ...
        for kx=1:size(tposes,3)
            ...
            nval = ncd7zip(qname,tname,tdir,3); % 3 equals ppm
            if (nval<dval)
                dval=nval;
            end
        end
